...
PyramidCompositor::PyramidCompositor(const std::string& input_pyramids_loc, const std::string& out_dir, const std::string& output_pyramid_name): 
    ...
    //     std::tie(_t_int, _c_int, _z_int) = ParseMultiscaleMetadata(axes_list, image_shape.size());
    //     _image_height = image_shape[image_shape.size()-2];
    //     _image_width = image_shape[image_shape.size()-1];
    //     if (_t_int.has_value()) {
    //         _num_tsteps = image_shape[_t_int.value()];
    //     } else {
    //         _num_tsteps = 1;
    //     }

    //     if (_c_int.has_value()) {
    //         _num_channels = image_shape[_c_int.value()];
    //     } else {
    //         _num_channels = 1;
    //     }

    //     if (_z_int.has_value()) {
    //         _image_depth = image_shape[_z_int.value()];
    //     } else {
    //         _image_depth = 1;
    //     }
    // }
...
void PyramidCompositor::WriteImageData(
    ...
    if (_t_int.has_value() && tsteps.has_value()) {
        output_transform = (std::move(output_transform) | tensorstore::Dims(_t_int.value()).ClosedInterval(tsteps.value().Start(), tsteps.value().Stop())).value();
        shape.emplace_back(tsteps.value().Stop() - tsteps.value().Start()+1);
    }

    if (_c_int.has_value() && channels.has_value()) {
        output_transform = (std::move(output_transform) | tensorstore::Dims(_c_int.value()).ClosedInterval(channels.value().Start(), channels.value().Stop())).value();
        shape.emplace_back(channels.value().Stop() - channels.value().Start()+1);
    }

    if (_z_int.has_value() && layers.has_value()) {
        output_transform = (std::move(output_transform) | tensorstore::Dims(_z_int.value()).ClosedInterval(layers.value().Start(), layers.value().Stop())).value();
        shape.emplace_back(layers.value().Stop() - layers.value().Start()+1);
    }

    output_transform = (std::move(output_transform) | tensorstore::Dims(_y_int).ClosedInterval(rows.Start(), rows.Stop()) |
                                                      tensorstore::Dims(_x_int).ClosedInterval(cols.Start(), cols.Stop())).value();
...
std::shared_ptr<std::vector<T>> PyramidCompositor::GetImageDataTemplated(const Seq& rows, const Seq& cols, const Seq& layers, const Seq& channels, const Seq& tsteps){
    ...
    int x_int=1, y_int=0;
    if (_t_int.has_value()){
        read_transform = (std::move(read_transform) | tensorstore::Dims(_t_int.value()).ClosedInterval(tsteps.Start(), tsteps.Stop())).value();
        x_int++;
        y_int++;
    }
    if (_c_int.has_value()){
        read_transform = (std::move(read_transform) | tensorstore::Dims(_c_int.value()).ClosedInterval(channels.Start(), channels.Stop())).value();
        x_int++;
        y_int++;
    }
    if (_z_int.has_value()){
        read_transform = (std::move(read_transform) | tensorstore::Dims(_z_int.value()).ClosedInterval(layers.Start(), layers.Stop())).value();
        x_int++;
        y_int++;
    }
